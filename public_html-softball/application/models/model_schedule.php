<?php
// $Id: model_schedule.php 252 2012-08-01 15:16:59Z Henry $
...
/* ... form the database query */
      $this->db->select( 'GameID, Date, Time, Diamond, HomeTeamID, HomeScore, VisitTeamID, VisitScore, Notes, Status, SportsVisitRating, SportsHomeRating, SportsVisitComments, SportsHomeComments, CommentsVisitOther, CommentsHomeOther' );
      if ($onlyScheduledGames) {
         $whereClause = "Status IN ('PLAYED', 'SCHEDULED', 'RAINOUT', 'FORFEIT-VISIT', 'FORFEIT-HOME') ";
       }
      if ($qualifier != "") {
...
/* ... if we didn't blow the query, get the data and then analyze the game result */
      ...
      if ($dbRow['Status'] == "PLAYED"  ||  $dbRow['Status'] == "FORFEIT-VISIT"  ||  $dbRow['Status'] == "FORFEIT-HOME") {
...
/* ... form the database query */
      ...
      if (!$allGames) {
         $this->db->where_in( 'Status', array( 'SCHEDULED', 'RAINOUT', 'PLAYED', 'FORFEIT-VISIT', 'FORFEIT-HOME' ) );
       }
      $this->db->order_by( 'Date', 'asc' );
      $this->db->order_by( 'Time', 'asc' );
      $this->db->order_by( 'Diamond', 'asc' );
...
/* ... form the database query */
      ...
      $whereClause = "(HomeTeamID = ".$teamID." OR VisitTeamID = ".$teamID.") AND Status IN ('PLAYED', 'FORFEIT-VISIT', 'FORFEIT-HOME')";
      $this->db->where( $whereClause );
...
/* ... build the query */
      ...
      $whereClause = "HomeTeamID IN (".$teamList.") AND VisitTeamID IN (".$teamList.") AND Status IN ('PLAYED', 'FORFEIT-VISIT', 'FORFEIT-HOME')";
      $this->db->where( $whereClause );
...
/*****
 * Function: findMissingSportsmanship (Find games that don't have results submitted)
 *
 * Arguments:
 *    $interval - number of days to forgive missing results
 *
 ...
 *
 *****/
   function findMissingSportsmanship( $interval ) {

/* ... data declarations */
      $gameList = array();

/* ... build the query for the data */
      $this->db->select( 'GameID, Date, Time, Diamond, HomeTeamID, VisitTeamID, SportsVisitRating, SportsHomeRating' );
      $where = "Date <= DATE_SUB( NOW(), INTERVAL ".$interval." DAY ) AND STATUS IN ('SCHEDULED', 'PLAYED', 'FORFEIT-VISIT', 'FORFEIT-HOME') ";
      $where .= "AND (SportsVisitRating IS NULL OR SportsHomeRating IS NULL)";
      $this->db->where( $where );
      $this->db->order_by( 'Date', 'asc' );
      $this->db->order_by( 'Time', 'asc' );
      $this->db->order_by( 'Diamond', 'asc' );
