...
// $Id: sched_scores.php 253 2012-08-01 15:31:58Z Henry $
// Last Change: $Date: 2012-08-01 11:31:58 -0400 (Wed, 01 Aug 2012) $
...
/* ... figure out the Forfeit state of the game */
		if ($data['gameDetails']['Status'] != "FORFEIT-VISIT"  &&  $data['gameDetails']['Status'] != "FORFEIT-HOME") {
			$data['gameDetails']['forfeit'] = "N";
		}
		elseif ($data['gameDetails']['Status'] == "FORFEIT-VISIT") {
			$data['gameDetails']['forfeit'] = "V";
		}
		elseif ($data['gameDetails']['Status'] == "FORFEIT-HOME") {
			$data['gameDetails']['forfeit'] = "H";
		}

/* ... need to figure out who is making the game submission for Sportsmanship info*/
		$data['gameDetails']['rating'] = 0;
		$data['gameDetails']['rating_text'] = "";
		$data['gameDetails']['open_comments'] = "";
		if ($this->Model_Team->isUserContactOnTeam( $data['gameDetails']['HomeTeamID'] )) {
      	if (!empty( $data['gameDetails']['SportsHomeRating'] )) {
      		$data['gameDetails']['rating'] = $data['gameDetails']['SportsHomeRating'];
      	}
      	$data['gameDetails']['rating_text'] = $data['gameDetails']['SportsHomeComments'];
      	$data['gameDetails']['open_comments'] = $data['gameDetails']['CommentsHomeOther'];
      }
		elseif ($this->Model_Team->isUserContactOnTeam( $data['gameDetails']['VisitTeamID'] )) {
      	if (!empty( $data['gameDetails']['SportsVisitRating'] )) {
      		$data['gameDetails']['rating'] = $data['gameDetails']['SportsVisitRating'];
      	}
     		$data['gameDetails']['rating_text'] = $data['gameDetails']['SportsVisitComments'];
			$data['gameDetails']['open_comments'] = $data['gameDetails']['CommentsVisitOther'];
      }
...
/* ... determine what set of fields are hidden from view when we show the form */
      if ($data['notPlayed']) {
         $data['pageJavaScript'] = "$(document).ready( function() { hideElement( '#visitScore' ); hideElement( '#l_visitScore' );
            hideElement( '#homeScore' ); hideElement( '#l_homeScore' ); hideElement( '#open_comments_div' ); hideElement( '#sportsmanship_div' );
            hideElement( '#forfeit_div' );} )\n";
       }
      else {
...
/*     the game was played or not */
      ...
      if ($this->input->post( "notPlayed" ) != "notPlayed") {
         ...
         $this->form_validation->set_rules( "rating", "Sportsmansnip Rating", "required|is_natural_no_zero" );
...
/* ... handle any factors relating to a forfeited game */
				if ($this->input->post( 'forfeit' ) != "N") {
					if ($this->input->post( 'forfeit') == "V") {
						$formData['HomeScore'] = 7;
						$formData['VisitScore'] = 0;
						$formData['Status'] = 'FORFEIT-VISIT';
					}
					elseif ($this->input->post( 'forfeit') == "H") {
						$formData['HomeScore'] = 0;
						$formData['VisitScore'] = 7;
						$formData['Status'] = 'FORFEIT-HOME';
					}
				}

/* ... need to figure out who is making the game submission for Sportsmanship info*/
				$gameDetails = $this->Model_Schedule->getGameDetails( $this->input->post( "gameID" ) );
				if ($this->Model_Team->isUserContactOnTeam( $gameDetails['HomeTeamID'] )) {
		      	$formData['SportsHomeRating'] = $this->input->post( 'rating' );
		      	$formData['SportsHomeComments'] = $this->input->post( 'rating_text' );
		      	$formData['CommentsHomeOther'] = $this->input->post( 'open_comments');
		      }
				elseif ($this->Model_Team->isUserContactOnTeam( $gameDetails['VisitTeamID'] )) {
		      	$formData['SportsVisitRating'] = $this->input->post( 'rating' );
		      	$formData['SportsVisitComments'] = $this->input->post( 'rating_text' );
		      	$formData['CommentsVisitOther'] = $this->input->post( 'open_comments');
		      }
...
/* ... data declarations */
      ...
      $sportsmanshipRatings = $this->config->item( 'sportsmanship_ratings' );
...
/* ... need to figure out who is making the game submission for Sportsmanship info*/
		...
		$data['gameDetails']['rating_text'] = "";
		if ($this->Model_Team->isUserContactOnTeam( $gameDetails['HomeTeamID'] )) {
      	if (!empty( $gameDetails['SportsHomeRating'] )) {
      		$gameDetails['rating'] = $gameDetails['SportsHomeRating'];
      	}
      	$gameDetails['rating_text'] = $gameDetails['SportsHomeComments'];
      }
		elseif ($this->Model_Team->isUserContactOnTeam( $gameDetails['VisitTeamID'] )) {
      	if (!empty( $gameDetails['SportsVisitRating'] )) {
      		$gameDetails['rating'] = $gameDetails['SportsVisitRating'];
      	}
     		$gameDetails['rating_text'] = $gameDetails['SportsVisitComments'];
     }
...
/* ... now build the body of the email message */
      if ($gameDetails['Status'] == "PLAYED"  ||  $gameDetails['Status'] == "FORFEIT-HOME"  ||  $gameDetails['Status'] == "FORFEIT-VISIT") {
         ...
         if ($gameDetails['Status'] == "FORFEIT-HOME") {
         	$body .= "\nNOTE: Game has been recorded as a forfeit by ".$this->Model_Team->getTeamName( $gameDetails['HomeTeamID'] )."\n";
         }
         elseif ($gameDetails['Status'] == "FORFEIT-VISIT") {
          	$body .= "\nNOTE: Game has been recorded as a forfeit by ".$this->Model_Team->getTeamName( $gameDetails['VisitTeamID'] )."\n";
         }
         $body .= "\nVisiting Team Open Comments:\n".$gameDetails['CommentsVisitOther']."\n";
         $body .= "\nHome Team Open Comments:\n".$gameDetails['CommentsHomeOther']."\n\n";
       }
 ...
 /*
  * For softball, ensure the webmaster gets a rainout note so rescheduling will occur
  */
         $bccAddr = array_merge( array( $this->config->item( 'my_replyAddress' ) ), $bccAddr );
       }
...
/* ... put in the standard details about disagreements */
      ...
      $body .= "If teams cannot reach a mutual agreement on the score, please send an email to ".$this->config->item( 'my_execAddress' )." providing ";
      ...
      $body .= "\n";
      $body .= "If you have concerns about the process for reporting games, please send them in an email to ".$this->config->item( 'my_replyAddress' )."\n";
      $body .= "\n\n";


/* ... we need a subject line */
      $subject = $this->config->item( 'mail_subject_prefix' )." Game report for ".$gameDetails['Date']." @ ".$gameDetails['Time']." on ".$gameDetails['Diamond']." [".$gameDetails['GameID']."]";

/* ... send the email message */
      $this->Model_Mail->sendTextEmail( $toAddr, $ccAddr, $bccAddr, $subject, $body );

/* ... if the game was played we also need to send out the Sportsmanship comments from the submitting team - and only back to themselves */
		if ($gameDetails['Status'] == "PLAYED"  ||  $gameDetails['Status'] == "FORFEIT-HOME"  ||  $gameDetails['Status'] == "FORFEIT-VISIT") {

/* ... need to figure out which team contact set for the game the user represents */
			$toAddr = array( $this->config->item( 'my_replyAddress' ) );
			if ($this->Model_Team->isUserContactOnTeam( $gameDetails['HomeTeamID'] )) {
				$toAddr = $homeEmails;
			}
			elseif ($this->Model_Team->isUserContactOnTeam( $gameDetails['VisitTeamID'] )) {
...
/* ... build the email message body */
			$body = "This email has been sent by an automated process and does not require a reply.\n\n";
			$body .= "Sportsmanship Rating And Comments\n";
			$body .= "=================================\n\n";
         $body .= "Date: ".$gameDetails['Date']." Time: ".$gameDetails['Time']." Diamond: ".$gameDetails['Diamond']."\n\n";
         $body .= $this->Model_Team->getTeamName( $gameDetails['VisitTeamID'] )." at ";
         $body .= $this->Model_Team->getTeamName( $gameDetails['HomeTeamID'] )."\n";
         $body .= "\nSportsmanship Rating For Opponents: (".$gameDetails['rating'].") ".$sportsmanshipRatings[$gameDetails['rating']]."\n\n";
         $body .= "Sportsmanship Comments About Opponents:\n".$gameDetails['rating_text']."\n";
         $body .= "\n-------------------------\n";
         $body .= "This email is only sent to your team as it is considered a private rating and commentary about another team. ";
         $body .= "This information may be viewed anonymously by the league executive who will act as we feel is appropriate. ";
         $body .= "\n\n";

/* ... we need a subject line */
 	   	$subject = $this->config->item( 'mail_subject_prefix' )." Sportsmanship report for ".$gameDetails['Date']." @ ".$gameDetails['Time']." on ".$gameDetails['Diamond']." [".$gameDetails['GameID']."]";
