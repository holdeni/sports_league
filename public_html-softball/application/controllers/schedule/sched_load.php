...
//               $entryData[0] = $this->config->item( "thisYear" )."-".date( "m-d", strtotime( trim( $entryData[0] ) ) );
               ...
               $entryData[0] = $dateTokens[2]."-".$dateTokens[0]."-".$dateTokens[1];
...
/* ... get the team IDs if there are teams playing in this game slot */
               ...
               $homeTeamID = -1;
               if ($entryData[4] != "") {
                  $visitTeamID = $this->Model_Team->getTeamID( trim( $entryData[3] ) );
                  $homeTeamID = $this->Model_Team->getTeamID( trim( $entryData[4] ) );
                  if ($visitTeamID == -1) {
                     print auto_typography( "**** ERROR: Unable to get team ID for ".$entryData[3]." [".$entryData[0]." @ ".$entryData[1]." on ".$entryData[2]."]\n" );
                   }
                  if ($homeTeamID == -1) {
